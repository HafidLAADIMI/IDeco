...
function Mur() {
  ...
  const handleNextStep = () => {
    if (!selectedSlide) {
      ...
      updateFormData({ formatGeneratedByAi: selectedSlide });
      setStep(9);
    }
  };
      ...
      <div className="flex flex-col gap-2 mt-3">
        ...
        <form className="flex px-3 text-black max-sm:text-xs sm:text-xl flex-col w-full justify-between items-start gap-6">
          <div className="w-full">
            <Carousel slides={slides} setSelectedSlide={setSelectedSlide} />
          </div>
          {error && <p className="text-red-500">{error}</p>}
          <button
            className="flex flex-row justify-center w-full text-black border border-orange rounded-lg items-center gap-3 py-2 px-10 cursor-pointer"
          >
